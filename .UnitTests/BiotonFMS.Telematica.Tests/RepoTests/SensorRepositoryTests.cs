...
public class SensorRepositoryTests
{
    ...
    public static IEnumerable<object[]> FilterTestData =>
        new []
        {
            ...
            {
                "Pagination Filter",
                new SensorsFilter { PageNum = 1, PageSize = 2 },
                new[] { 1, 2 }
            },
            new object[]
            {
                "Tracker Filter 1",
                new SensorsFilter { TrackerId = 1},
                new[] { 1, 2, 4 }
            },
            new object[]
            {
                "Tracker Filter 2",
                new SensorsFilter { TrackerId = 2 },
                new[] { 3 }
            },
            new object[]
            {
                "Sorting Ascending",
                new SensorsFilter { SortBy = SensorSortBy.Name },
                new[] { 2, 3, 4, 1 }
            },
            new object[]
            {
                "Sorting Descending",
                new SensorsFilter { SortBy = SensorSortBy.Name, SortDirection = SortDirection.Descending },
                new[] { 1, 4, 3, 2 }
            }
        };
    ...
    public static IEnumerable<object[]> ConstraintViolationTestData =>
        new []
        {
            new object[]
            {
                ...
                new Sensor() { Id = -1, TrackerId = 1, Name = "Sensor 2" },
                "*Датчик с именем * уже существует*"
            },
            new object[]
            {
                "[POSITIVE] Duplicate Name Inside Tracker 2",
                new Sensor() { Id = -1, TrackerId = 2, Name = "Sensor 3" },
                "*Датчик с именем * уже существует*"
            },
            new object[]
            {
                "[NEGATIVE] Globally Duplicate Name",
                new Sensor() { Id = -1, TrackerId = 1, Name = "Sensor 3" },
                ""
            },
        };
    ...
    private static SensorRepository CreateSensorRepository(ICollection<Sensor> sensorList)
    {
        ...
        var repo = new SensorRepository(LoggerMock.GetSensorRepositoryStub(), keyValueProviderMock, sensorQueryProviderMock, unitOfWorkFactoryMock);
        return repo;
    }
